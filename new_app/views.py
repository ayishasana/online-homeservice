...
from new_app.forms import register_form, ScheduleForm, Login_form, AddBill, creditcardform, FeedbackForm, \
    register_form1, work_form
from new_app.models import Take_appointment, register, Schedule_add, Bill, register1, CreditCard, complaints, work
...
def update(request,id):
    ...
    if request.method == 'POST':
        form = register_form(request.POST,request.FILES,instance=a)
        if form.is_valid():
            form.save()
            return redirect('workers_data')
...
def view_filtered_schedule(request):
    ...
    data= Schedule_add.objects.filter(worker=u)
    return render(request,"worker/worker_view_schedule.html",{"data":data})
...
@login_required(login_url='loginpage')
def view_filtered_workersdata(request):
    data=register.objects.filter(user=request.user)
    print(data)
    return render(request,"worker/view_filtered_workersdata.html",{"data": data})
...
@login_required(login_url='loginpage')
def update_workersdata(request,id):
    a = register.objects.get(id=id)
    form = register_form(instance=a)
    if request.method == 'POST':
        form = register_form(request.POST,request.FILES,instance=a)
        if form.is_valid():
            form.save()
            return redirect('workerview_workersdata')

    return render(request,'worker/update_workersdata.html',{'form':form})
